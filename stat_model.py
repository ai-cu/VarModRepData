...
def process_simple(img_signals, constellation):
    #constellation = [1 + 1j, 1 - 1j, -1 + 1j, -1 - 1j]
    bin_data = []
    for symbol in img_signals:
        closest_symbol = min(constellation, key=lambda x: abs(x - symbol))

        bin_data.append(constellation.index(closest_symbol))
        #bin_data.extend([int(bit) for bit in format(symbol_index, '02b')])
    return bin_data

def process_QPSK_image_stream_LRU(img_signals, expected, constellation):
    bin_data = []
    exp_ct = 0
    exp = expected.copy()
    for symbol in img_signals:
...
def process_QPSK_image_stream_PROB(img_signals, expected, constellation, prob):
    bin_data = []
    exp_ct = 0
    exp = expected.copy()
    for symbol in img_signals:
